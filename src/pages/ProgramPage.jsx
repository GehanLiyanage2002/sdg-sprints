    ...
    <div className="pt-32 pb-24 min-h-screen relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <h2 className="text-5xl md:text-6xl font-black text-slate-900 dark:text-white mb-6 tracking-tight drop-shadow-lg">
        ...
        <div className="mb-32">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-10 gap-6">
            <div>
              <h3 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-3">Capacity Building Workshops</h3>
              <p className="text-slate-600 dark:text-rose-100/70 max-w-2xl text-lg">
            ...
            {/* Quick Stats Badge */}
            <div className="flex flex-col gap-2 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-4 rounded-2xl backdrop-blur-md shrink-0">
              <div className="flex items-center gap-3 text-slate-900 dark:text-white">
                <svg className="w-5 h-5 text-rose-600 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {workshopTopics.map((topic, idx) => (
              <div key={idx} className="flex items-start gap-4 bg-white dark:bg-white/5 border border-slate-100 dark:border-white/5 p-6 rounded-2xl hover:bg-slate-50 dark:hover:bg-white/10 dark:bg-[#24050b] hover:border-rose-500/30 transition-all duration-300 group">
                <div className="mt-1 w-6 h-6 rounded-full bg-rose-500/20 flex items-center justify-center shrink-0 group-hover:bg-rose-500/40 transition-colors">
                  <svg className="w-4 h-4 text-rose-600 dark:text-rose-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                </div>
                <p className="text-slate-700 dark:text-rose-100/80 font-medium leading-relaxed group-hover:text-slate-900 dark:text-white transition-colors">
                  {topic}
                </p>
              </div>
            ))}
        ...
        <div>
          <div className="text-center mb-16">
            <h3 className="text-3xl md:text-4xl font-bold text-slate-900 dark:text-white mb-4">SDG Solutions Challenge</h3>
            <p className="text-slate-600 dark:text-rose-100/70 max-w-2xl mx-auto text-lg">
              ...
              {timelineEvents.map((event, idx) => (
                <div key={idx} className={`relative flex flex-col md:flex-row items-start ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''} group`}>
                  ...
                  <div className={`w-full md:w-1/2 pl-12 md:pl-0 ${idx % 2 === 0 ? 'md:pr-16 text-left md:text-right' : 'md:pl-16 text-left'}`}>
                    <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-8 rounded-3xl backdrop-blur-md hover:bg-slate-50 dark:hover:bg-white/10 dark:bg-[#24050b] hover:border-rose-400/50 transition-all duration-300 shadow-xl relative overflow-hidden">
                      
                      {/* Subtle background number/step */}
                      <div className={`absolute -top-4 -right-4 text-7xl font-black text-slate-900 dark:text-white/5 select-none pointer-events-none ${idx % 2 === 0 ? 'md:right-auto md:-left-4' : ''}`}>
                        {idx + 1}
                      </div>

                      <span className="inline-block px-3 py-1 rounded-full bg-rose-500/20 text-rose-600 dark:text-rose-400 text-xs font-bold uppercase tracking-wider mb-3">
                        {event.step}
                      </span>
                      
                      <h4 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">{event.title}</h4>
                      ...
                      <div className={`flex flex-wrap items-center gap-3 mb-4 text-sm font-medium ${idx % 2 === 0 ? 'md:justify-end' : 'justify-start'}`}>
                        ...
                        <span className="text-slate-900 dark:text-white/30 hidden sm:inline">•</span>
                        <span className="flex items-center gap-1.5 text-slate-500 dark:text-rose-200/70">
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                          {event.duration}
                        </span>
                      </div>
                      
                      <p className="text-slate-600 dark:text-rose-100/70 leading-relaxed mb-5 relative z-10">
                        {event.desc}
                      </p>
