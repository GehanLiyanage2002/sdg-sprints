export default function CommitteePage() {
  ...
  return (
    <div className="pt-32 pb-24 min-h-screen relative overflow-hidden">
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[800px] bg-rose-200/40 dark:bg-red-600/10 rounded-full blur-[150px] pointer-events-none mix-blend-screen"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-lg md:text-xl text-slate-600 dark:text-rose-100/70 max-w-2xl mx-auto">
          ...
          {teams.map((team, teamIndex) => (
            <div key={teamIndex} className="relative">
              ...
              <div className="flex items-center gap-4 mb-10 justify-center md:justify-start">
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white">{team.teamName}</h3>
                <div className="h-px bg-gradient-to-r from-rose-500/50 to-transparent flex-grow hidden md:block"></div>
              </div>

              {/* Members Grid */}
              <div className={`grid grid-cols-1 sm:grid-cols-2 ${team.teamName === "Leadership" ? "lg:grid-cols-5" : "lg:grid-cols-4"} gap-6 justify-items-center`}>
                {team.members.map((member, memberIndex) => (
                  <div 
                    key={memberIndex} 
                    className="group relative flex flex-col items-center text-center rounded-3xl bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 p-8 backdrop-blur-sm hover:bg-slate-50 dark:hover:bg-white/10 dark:bg-[#24050b] transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(225,29,72,0.1)] hover:border-rose-500/30 w-full"
                  >
                    ...
                    <span className="inline-block px-4 py-1.5 rounded-full bg-rose-500/10 border border-rose-500/20 text-rose-600 dark:text-rose-400 text-sm font-medium">
                      {member.role}
                    </span>
                  </div>
                ))}
